...
//                          accent squiggle (mirrors the marketing-site hero).
//
...
// Hand-drawn accent squiggle under an emphasis phrase — same path the hero uses.
...
// The wordmark is the app's own SVG rather than a copied raster, so the
// thumbnail cannot drift from the logo the site ships.
const logoDataUri = `data:image/svg+xml;base64,${readFileSync(
    join(repoRoot, 'public/images/lua/full-black.svg'),
).toString('base64')}`;
...
const tmpFile = join(tmpdir(), `lua-thumbnail-${process.pid}.html`);
