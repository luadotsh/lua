#!/bin/sh
# Lua — container entrypoint. Idempotent first-run setup, then exec supervisord.

set -e

cd /var/www/html

TARGET="${LUA_TARGET:-dev}"
...
if [ "${LUA_DOCKER_BOOTSTRAP:-0}" = "1" ] && [ ! -f .env ]; then
...
if [ "${LUA_SKIP_BOOTSTRAP:-0}" = "1" ]; then
    echo "[entrypoint] LUA_SKIP_BOOTSTRAP=1 — exec'ing supervisord without setup"
